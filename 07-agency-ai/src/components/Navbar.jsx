...
function Navbar({theme,setTheme}) {
    ...
    return (
        <motion.div
            ...
            <div className={`text-gray-700 dark:text-white sm:text-sm ${!slidebarOpen ?'max-sm:w-0 overflow-hidden ':'max-sm:w-60 max-sm:pl-10 '} max-sm:fixed top-0 bottom-0 right-0 max-sm:min-h-screen max-sm:h-full max-sm:flex-col max-sm:bg-primary max-sm:text-white max-sm:pt-20 flex sm:items-center gap-5 transition-all`}>
            ...
            <div className="flex items-center gap-2 sm:gap-4">
                ...
                <img alt="" className="w-8 sm:hidden" src={theme=== "dark" ? assets.menu_icon_dark : assets.menu_icon} onClick={()=>setSidebarOpen(true)}/>
                ...
                <a href="#contact-us" className="text-sm max-sm:hidden flex items-center gap-2 bg-primary text-white px-6 py-2 rounded-full cursor-pointer hover:scale-103 transition-all">
                    Connect
                    <img width="14" alt="" src={assets.arrow_icon}/>
                </a>
            </div>
        </motion.div>
    );
