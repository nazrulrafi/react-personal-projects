        ...
        <div className="h-screen overflow-scroll flex flex-col items-start justify-between md:p-8 md:p-0 p-4 pt-8 pb-0">
            <form onSubmit={handleSubmit}>
                <div className="flex flex-col gap-1">
                    <p>Course Title</p>
                    <input type="text" onChange={e => setCourseTitle(e.target.value)} value={courseTitle} placeholder="Type Here" className="outline-none md:pay-2.5 py-2 px-3 rounded border border-gray-500" required/>
                </div>
                <div className="flex flex-col gap-1">
                ...
                <div className="flex items-center justify-between flex-wrap">
                    <div className="flex flex-col gap-1">
                        ...
                        <input type="number" onChange={e => setCoursePrice(e.target.value)} value={coursePrice}
                               className="outline-none md:pay-2.5 py-2 w-28 px-3 rounded border border-gray-500" required/>
                    </div>
                ...
                <div>
                                ...
                                <div className="p-4">
                                    {chapter.chapterContent.map((lecture, index) => (
                                        <div key={index} className="flex justify-between items-center mb-2">
                                            <span>{index + 1} {lecture.lectureTitle} - {lecture.lectureDuration} mins - <a
                                                href={lecture.lectureUrl} target="_blank"
                                                className="text-blue-500">Link</a> - {
                                                lecture.isPreviewFree ? "Free Preview" : "Paid"
                                                }</span>
                                            <img onClick={()=> handleLecture("remove",chapter.chapterId,index)} src={assets.cross_icon} className="cursor-pointer" alt=""/>
                                        </div>
                                    ))}
                    ...
                    <div onClick={()=> handleChapter("add")} className="flex justify-center mt-5 items-center bg-blue-100 p-2 rounded-lg cursor-pointer">+ Add Chapter</div>

                    {
                        showPopup && (
                            <div className={`flex inset-0 fixed items-center justify-center bg-gray-800 bg-opacity-50`}>
                                <div className={`bg-white text-gray-700 p-4 rounded relative w-full max-w-80`}>
                                    ...
                                    <button type="button" className="w-100 bg-blue-400 text-white px-4 py-2 rounded" onClick={addLecture}>Add</button>
